...
```bash
# 安装控制面 Agent（推荐；包装器在 ~/Library/Application Support/newsc）
bash scripts/deploy/install-cloud-bridge-launchd.sh
# 手动拉一次（marks + outbox + sources 对账）
bash scripts/deploy/pull-cloud-control.sh
```

**iCloud 注意**：仓库若在 `iCloud Drive` / `Mobile Documents` 下，launchd 直接执行仓库内 `.sh` 或 `source` 仓库内 `.env` 会被 TCC 拒绝（`Operation not permitted` / exit 126）。`cloud-bridge` / `push-db` / `pipeline-sources` 均经 `~/Library/Application Support/newsc/run-*.sh` 启动；`.env.cloud.local` 在安装时复制到同目录，由包装器加载。

**sources 对账**：`cloud_bridge` 每次会把云端 `sources` 按 id 下行到本机（不覆盖本机 cursor）。避免「云端订阅了 YouTube、本机无源 → 不采集 → push 再盖掉云端源」。
...
定时推送 LaunchAgent 实际跑的是 `~/Library/Application Support/newsc/deploy/` 下的脚本副本（避免 iCloud TCC 导致 exit 126）。改仓库内 `push-db-to-cloud.sh` 后需重新 `install-push-db-launchd.sh` 才会同步到副本。
